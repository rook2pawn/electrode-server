...
function startElectrodeServer(context) {
  ...
  const registerPlugins = Promise.method(async (plugins) => {
...
/*
      ...
      .catch(Promise.TimeoutError, () => {
        return new Promise((resolve, reject) => {
          server.stop(() => {
            reject(new Error("Electrode Server register plugins timeout.")); // eslint-disable-line
          });
        });
      });
      */
  ...
  const startServer = Promise.method(async () => {
    try {
      await server.start();
    } catch (e) {
      throw new Error(e);
...
module.exports = function electrodeServer(appConfig, decors, callback) {
  ...
  const makeHapiServerConfig = (context) => {
    ...
    _.assign(hapiServerConfig, context.config.connection);
  ...
  const start = (context) => {
    var ctx = makeHapiServerConfig(context);
    return Promise.try(() => Hapi.Server(makeHapiServerConfig(context)))
      .tap(() => emitEvent(context, "server-created"))
      .then((server) => {
